...
module.exports = {
   createTransaction:async (req, res) => {
    try{
        const { totalAmount,paidAmount,transactionId,amexorderId,paymentMethod,idForPayment} = req.body;
        ...
        if(firstTwoChars === "SO"){
            ...
            const transactionInsertQuery = `insert into transaction (sales_order_Id,paymentMethod ,transactionId,totalAmount,paidAmount,amex_order_Id) values(${salesOrderResponse[0].id},"${paymentMethod}","${transactionId}",${totalAmount},${paidAmount},"${amexorderId}")`
        ...
        if(firstTwoChars === "IN"){
            ...
            const transactionInsertQuery = `insert into transaction (invoiceId,paymentMethod ,transactionId,totalAmount,paidAmount,amex_order_Id) values(${invoiceIDQueryResponse[0].id},"${paymentMethod}","${transactionId}",${totalAmount},${paidAmount},"${amexorderId}")`
            const insetTransatction = await query(transactionInsertQuery);
